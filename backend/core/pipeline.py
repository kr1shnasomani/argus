...
from services.llm import generate_evidence_card, generate_resolution_message, generate_text
...
async def process_ticket(
    ...
    async def conclude(status: str, action: GateAction, reason: str, extra: dict = None) -> Dict[str, Any]:
        ...
        signals = conf_report.get("signals", {}) if conf_report else {"A": None, "B": None, "C": None}
        ...
        outcome_data = {
            ...
            "signal_a": signals.get("A", {}).get("score") if isinstance(signals.get("A"), dict) else (signals.get("A").score if signals.get("A") else None),
            "signal_b": signals.get("B", {}).get("score") if isinstance(signals.get("B"), dict) else (signals.get("B").score if signals.get("B") else None),
            "signal_c": signals.get("C", {}).get("score") if isinstance(signals.get("C"), dict) else (signals.get("C").score if signals.get("C") else None),
            "escalation_reason": reason if status != "auto_resolved" else None,
            "agent_verified": None,
            "ai_suggestion": None if status == "auto_resolved" else (
                qdrant_results[0].get("resolution") if qdrant_results and isinstance(qdrant_results[0], dict) else None
            ),
        ...
        try:
            ...
            if status == "auto_resolved":
                evidence_card_dict = {
                    ...
                    "signals": {k: (v.model_dump() if hasattr(v, "model_dump") else v) for k, v in signals.items()} if signals else {},
                    "sandbox_passed": sandbox_passed, 
    ...
    if not ticket.get("category"):
        ...
        try:
            ...
            valid_categories_res = supabase_client.table("category_thresholds").select("category").execute()
            valid_categories = [row["category"] for row in valid_categories_res.data] if valid_categories_res.data else []
            ...
            detected_category = await generate_text(f"Ticket Description: {ticket.get('description', '')}", sys_prompt)
            ...
            if matched_category:
                ...
                logger.warning(f"AI returned invalid category '{detected_category}'. Valid categories: {valid_categories}")
            ...
            logger.error(f"Category auto-detection failed: {e}. Trying keyword-based fallback...")
            ...
            keyword_map = {
                ...
                "Software Install": ["install", "software", "app", "license", "uninstall"],
                "Network/Connectivity": ["network", "internet", "wifi", "connected", "connectivity", "dns", "gateway", "ethernet"],
                "Permissions/Access": ["access", "permission", "unauthorized", "denied", "share", "folder", "group"],
                "Auth/SSO": ["password", "sso", "mfa", "2fa", "authenticat", "locked", "unlock"],
            }
        ...
        try:
            supabase_client.update_ticket(ticket["id"], {"category": ticket["category"]})
        except Exception:
            pass
    ...
    try:
        ...
        return await conclude("escalated", GateAction.ESCALATE, f"Embedding generation failed: {e}")
    ...
    try:
        ...
        return await conclude("escalated", GateAction.ESCALATE, f"Vector DB retrieval failed: {e}")
    ...
    if novelty_res.action == GateAction.ESCALATE:
        ...
        return await conclude("escalated", GateAction.ESCALATE, novelty_res.reason, extra_state)
    ...
    if conf_report.decision == ConfidenceDecision.ESCALATE:
        return await conclude("escalated", GateAction.ESCALATE, conf_report.reason, extra_state)

    # --- Layer 4: Resolution Mapping & Sandbox Execution ---
    logs.append("Running Layer 4: Action Mapping & Sandbox")
    action_payload = map_resolution_to_action(qdrant_results[0], cluster_map, ticket.get("user_email", "unknown@domain.com"), ticket.get("category", ""))
    ...
    if not sandbox_res.get("success", False):
        return await conclude("escalated", GateAction.ESCALATE, f"Sandbox validation failed: {sandbox_res.get('error', 'Unknown Error')}", extra_state)
    ...
    try:
        resolution_msg = await generate_resolution_message(resolution_text, ticket_description)
    except Exception as e:
        logger.warning(f"Resolution message generation failed: {e}. Using raw resolution text.")
